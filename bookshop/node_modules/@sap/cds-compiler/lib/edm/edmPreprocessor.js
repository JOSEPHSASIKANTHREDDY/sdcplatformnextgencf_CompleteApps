...
*/
  ...
  function initializeStructure(def) {
    ...
    forEachGeneric(structParent, 'elements', (element, elementName) => {
      ...
      if(element._parent && element._parent.$mySchemaName && !isODataSimpleIdentifier(elementName)) {
      ...
      if(isAssociationOrComposition(element) || element._ignore) {
        if(!element._target) {
          error(null, ['definitions', def.name, 'elements', elementName],
            'Expect association target to be resolvable');
          return;
        }
  ...
  */
  function exposeTargetsAsProxiesOrSchemaRefs(struct) {
        ...
        if(isProxyRequired(element)) {
          if(isBetaEnabled(options, 'odataProxies') &&
            ...
            if(!proxy) {
              ...
              // if a new proxy could be created, 
              // try to register it by prepending the service schema name
    ...
    function createProxyFor(assoc, targetSchemaName) {
      ...
      if(Object.keys(proxy.$keys).length === 0) {
        info(null, ['definitions', assoc._parent.name, 'elements', assoc.name],
          `No proxy entity type created for unexposed association target "${assoc._target.name}": Target has no primary keys`);
        return undefined;
      }
      return proxy;
    ...
    function registerProxy(proxy, element) {
      ...
      if(proxy.kind === 'entity') {
        ...
        if(!alreadyRegistered) {
          ...
          `Created proxy entity type "${proxy.name}" for unexposed association target "${element._target.name}"`);
        }
          ...
          warning(null, ['definitions', element._parent.name, 'elements', element.name],
            `No proxy entity type created for unexposed association target "${element._target.name}": Name collision with "${proxyName}" of kind "${alreadyRegistered.kind}"`);
          return undefined;
        }
      }
      else {
        ...
        if(!csn.definitions[schemaName]) {
          ...
          schemaNames.push(schemaName);
          info(null, ['definitions', struct.name, 'elements', element.name],
            `Created namespace reference "${proxy.name}" for unexposed association target "${element._target.name}"`);
        }
  ...
  */
  ...
  function checkArtifactIdentifierAndBoundActions(artifact) {
    const isInService = artifact.$edmSchemaIgnore ? artifact.$edmSchemaIgnore !== artifact.name : true;
    if(artifact.$mySchemaName && isInService) {
