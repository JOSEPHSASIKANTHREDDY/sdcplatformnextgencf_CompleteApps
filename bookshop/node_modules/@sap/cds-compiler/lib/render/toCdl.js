...
function toCdsSource(model, options){
  ...
  return toCdsSourceCsn(csn, mergeOptions(csn.options, options));
}
...
function toCdsSourceCsn(csn, options) {
  ...
  const {artifactRef} = require('../model/csnRefs').csnRefs(csn);
  ...
  function renderEventIfCDLMode(artifactName, art, env){
    if(cdlMode){
      ...
      if (art.query) {
        result += ' : ';
        result += renderQuery(art.query, true, 'projection', env,
...
*/
  ...
  function renderElement(elementName, elm, env, duplicateChecker) {
    ...
    duplicateChecker && duplicateChecker.addElement(quotedElementName, elm && elm.$location, elementName);
    result += env.indent + (elm.virtual ? 'virtual ' : '')
                         + (elm.key ? 'key ' : '')
                         + ((elm.masked && !elm._ignoreMasked)? 'masked ' : '')
                         + quotedElementName + (omitColon ? ' ' : ' : ')
                         + renderTypeReference(elm, env, undefined, true)
  ...
  function renderViewSource(source, env) {
    ...
    // JOIN
      ...
      for (let i = 1; i < source.args.length; i++) {
        ...
        if(!options.forHana) // HANA CDS doesn't understand join cardinality
    ...
    // Ordinary path, possibly with an alias
      ...
      return renderAbsolutePathWithAlias(source, env, cdlMode);
    }
  }
   ...
   * @param {object} path
   * @param {object} env
   * @param {boolean} joinWithColon
   * @return {string}
   */
  function renderAbsolutePath(path, env, joinWithColon) {
    // Sanity checks
    if (!path.ref) {
    ...
    if (path.ref.length > 1) {
      result += `${joinWithColon ? ':' : '.'}${renderExpr({ref: path.ref.slice(1)}, env)}`;
    }
    return result;
  }
   ...
   * @param {object} path
   * @param {object} env
   * @param {boolean} renderColon Render as explicit artifact reference with a :
   * @return {string}
   */
  function renderAbsolutePathWithAlias(path, env, renderColon) {
    let result = renderAbsolutePath(path, env, renderColon);
  ...
  function renderViewColumn(col, env) {
    ...
    if(options.forHana && element && element.virtual) {
      const renderVirtual = isBetaEnabled(options, 'dontRenderVirtualElements') ? !!options.forHana.renderVirtualElements : true;
      if(renderVirtual)
      ...
      const key = (!env.skipKeys && (col.key || (options.forHana && element && element.key)) ? 'key ' : '');
  ...
  function renderParameter(parName, par, env) {
    ...
    if(cdlMode && par.default) {
      result += ' default ' + renderExpr(par.default);
    }
  ...
  function renderTypeOrAnnotation(artifactName, art, env) {
    ...
    if (art.elements && !art.type) {
      ...
      // Derived type or annotation with non-anonymous type
      result += ' : ' + renderTypeReference(art, env, false, true) + ';\n';
    }
    return result;
  }
   ...
   * @param {boolean} [renderWithColon=false]
   * @return {string}
   */
  function renderTypeReference(elm, env, noEnum=false, renderWithColon=cdlMode) {
    ...
    if (elm.type === 'cds.Association' || elm.type === 'cds.Composition') {
      ...
      if(elm.target || elm.targetAspect && typeof elm.targetAspect === 'string') {
        ...
        result += '{\n';
        for (let name in elm.targetAspect.elements) {
    ...
    if (elm.type.ref && cdlMode) {
      if (elm.enum) {
        const source = artifactRef(elm.type);
        if(!source.enum){
            // enum was defined at this element and not at the referenced one
          result += renderAbsolutePath(elm.type, env, renderWithColon) + renderEnum(elm.enum, env);
        } else {
          result += renderAbsolutePath(elm.type, env, renderWithColon);
        }
      } else {
        result += renderAbsolutePath(elm.type, env, renderWithColon);
      }
      return result;
    } else if(elm.type.ref && options.forHana) {
      // For HANA CDS, we need a 'type of'
      result += 'type of ' + renderAbsolutePath(elm.type, env, false);
      if (elm.enum) {
  ...
  function renderExpr(x, env, inline=true, inExpr=false) {
    ...
    if (x instanceof Array) {
      ...
      return x.map(item => renderExpr(item, env, inline, inExpr)).join(' ');
    }
    ...
    function renderPathStep(s, idx) {
      ...
      // ID with filters or parameters
        ...
        if (s.where) {
          ...
          result += `[${s.cardinality ? (s.cardinality.max + ': ') : ''}${renderExpr(s.where, env, inline, inExpr)}]`;
        }
        return result;
      }
      else {
  ...
  function quoteOrUppercaseId(id, location=null) {
    if (plainNames) {
      ...
      if (keywords.hana.includes(result)) {
        signal(warning`The identifier "${id}" is a HANA keyword.`, location);
      }
      return result;
    } else {
      return quoteId(id);
